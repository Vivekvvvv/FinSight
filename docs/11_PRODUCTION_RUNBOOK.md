...
> 最后更新：2026-06-01
> 适用范围：当前 Python FastAPI + Vue 默认生产链路
> SSOT：`docs/DOCS_INDEX.md`
...
- 前端入口：`frontend-vue` 构建出的 Vue SPA，经 Nginx 反代到 Python FastAPI

不再把 legacy router/supervisor、旧 React 前端或 Spring Boot BFF 作为生产主链路。

---

## 2. 发布门禁（必须全部通过）

在发布前，必须完成以下门禁。Windows 优先使用 PowerShell 脚本，Linux/macOS 使用 Bash 脚本；手工命令仅用于排障时拆分定位。

```bash
python scripts/check_cutover_map.py
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/release_gate.ps1
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/release_gate.ps1 -WithE2E
...
npm run lint --prefix frontend-vue
...
将 `frontend-vue/dist` 交给静态资源服务（Nginx/对象存储/CDN）。
...
> 当前工作区没有 `scripts/checkpointer_switch_drill.py` 自动化脚本；不要把旧命令作为当前门禁执行。上线前需要恢复等价脚本，或由运维手动完成切换演练并按本节产出证据。
...
## 12. Stability & Security Closure Evidence

旧版 `scripts/t4_stability_security_closure.py` 自动化脚本已不在当前工作区；不要把它作为当前发布门禁。当前发布候选必须通过 CI / release gate 和人工演练证据共同闭环。
...
```
### 13.4 本地 Stage4 Postgres Gate 联调
...
> 详细状态见 `docs/MIGRATION_SPRING_VUE_PLAN.md` 与 `docs/SPRING_VUE_MIGRATION_ACCEPTANCE.md`。Spring Boot BFF 已从默认链路移除。
...
- `backend` — Python FastAPI, 承载 AI、RAG、LangGraph、LLM、SSE 和业务 API。
- `frontend` — `frontend-vue` 构建出的 Vue SPA + nginx, 直接反代到 `backend:8000`。

默认访问:

- 前端: `http://localhost`
- 后端: 默认仅在 Compose 内网以 `backend:8000` 暴露; 本地调试需叠加 `docker-compose.dev.yml` 后访问 `http://localhost:8000`
...
门禁覆盖 FastAPI import smoke、Python 后端测试、Vue lint/typecheck/build。需要浏览器 E2E 时追加 `-WithE2E`。
...
脚本保留旧文件名, 现在用于校验默认链路仍是 `frontend-vue -> backend/FastAPI`。

### 14.4 回滚

| 场景 | 动作 | 时效 |
|---|---|---|
| Vue 前端异常 | 回退到上一版 frontend 镜像或 nginx 配置 | 取决于镜像/配置发布方式 |
| Python 后端异常 | 回退到上一版 backend 镜像 | 取决于镜像发布方式 |
| 数据异常 | 使用 PostgreSQL / volume 备份恢复 | 取决于备份策略 |

...
@pytest.mark.parametrize("parameter", ["market", "operation", "analysis_depth", "output_mode"])
def test_tools_capabilities_rejects_oversized_query_parameters(parameter):
    with TestClient(app) as client:
        response = client.get(
            "/api/tools/capabilities",
            params={parameter: "x" * 65},
        )

...
@pytest.mark.asyncio
async def test_news_agent_cache_key_is_ticker_scoped(mock_llm, mock_cache, mock_tools, circuit_breaker):
    agent = NewsAgent(mock_llm, mock_cache, mock_tools, circuit_breaker)

    await agent._initial_search("News for AAPL", "AAPL")
    await agent._initial_search("Latest Apple headlines", "AAPL")

    cache_get_keys = [call.args[0] for call in mock_cache.get.call_args_list]
    cache_set_keys = [call.args[0] for call in mock_cache.set.call_args_list]

    assert cache_get_keys == ["AAPL:news:24h", "AAPL:news:24h"]
    assert cache_set_keys == ["AAPL:news:24h", "AAPL:news:24h"]

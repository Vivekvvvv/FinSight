...
@pytest.mark.asyncio
async def test_deep_search_cache_key_uses_stable_digest():
    mock_cache = MagicMock()
    mock_cache.get.return_value = None
    mock_tools = MagicMock()
    agent = DeepSearchAgent(None, mock_cache, mock_tools)
    ...
    await agent._initial_search("NVDA investment thesis", "NVDA")

    expected_digest = hashlib.sha256("NVDA investment thesis".encode("utf-8")).hexdigest()[:16]
    mock_cache.get.assert_called_once_with(f"NVDA:deep_search:{expected_digest}")


@pytest.mark.asyncio
async def test_deep_search_initial_search_falls_back_to_snippet_docs_when_fetch_empty():
    mock_cache = MagicMock()
    mock_cache.get.return_value = None
    mock_tools = MagicMock()
    agent = DeepSearchAgent(None, mock_cache, mock_tools)
    agent._search_web = MagicMock(return_value=[
        {
            "title": "AAPL margin update",
            "url": "https://example.com/aapl-margin",
            "snippet": "Margins expanded after service revenue mix improved.",
            "source": "search",
        }
    ])
    agent._fetch_documents = MagicMock(return_value=[])

    docs = await agent._initial_search("AAPL margin outlook", "AAPL", queries=["AAPL margin"])

    assert len(docs) == 1
    assert docs[0]["title"] == "AAPL margin update"
    assert docs[0]["content"] == "AAPL margin update\nMargins expanded after service revenue mix improved."
    assert docs[0]["search_query"] == "AAPL margin"
    assert docs[0]["deepsearch_phase"] == "initial"
    assert docs[0]["degraded"] is True
    assert docs[0]["degrade_reason"] == "snippet_only"
    assert mock_cache.set.call_count == 1


@pytest.mark.asyncio
async def test_deep_search_rag_observability_failure_does_not_block_research(monkeypatch):
    ...
    monkeypatch.setattr("backend.rag.get_rag_service", lambda: FailingRAGService())
    ...
    docs = [
        {
            ...
            "snippet": "AAPL source snippet.",
            "content": "AAPL source content with enough context for observability.",
            "source": "search",
            "is_pdf": False,
        }
    ]
    agent._initial_search = AsyncMock(return_value=docs)
    agent._first_summary = AsyncMock(return_value="research summary")
    agent._identify_gaps = AsyncMock(return_value=[])

    result = await agent.research("AAPL research", "AAPL")
...
def test_search_web_supplements_authoritative_feeds_when_trusted_results_insufficient():
    mock_tools = MagicMock()
    ...
    mock_tools.search_authoritative_feeds.return_value = [
        {
...
def test_search_web_does_not_use_real_authoritative_feed_without_injected_tool(monkeypatch):
    tools = SimpleNamespace(
        TAVILY_API_KEY="",
        TAVILY_AVAILABLE=False,
        EXA_API_KEY="",
        EXA_AVAILABLE=False,
        search=lambda _query: "fallback search",
    )
    agent = DeepSearchAgent(None, MagicMock(), tools)
    agent._parse_search_text = MagicMock(
        return_value=[
            {
                "title": "Untrusted source",
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: pytest.fail("real authoritative feed must not be called"),
    )

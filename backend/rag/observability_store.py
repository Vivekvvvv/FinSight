...
def _project_db_browser_columns(columns: list[str], *, include_raw: bool) -> list[str]:
    if include_raw:
        return columns
    return [column for column in columns if column not in _RAG_RAW_COLUMNS]
...
class SQLRAGObservabilityStore(_RuntimeSQL):
    ...
    def browse_db_table(self, *, table_name: str, limit: int = 50, offset: int = 0, q: str | None = None, collection: str | None = None, run_id: str | None = None, source_doc_id: str | None = None, include_raw: bool = False, role: str = "reader") -> dict[str, Any]:
        ...
        with self._engine.connect() as conn:
            ...
            selected_columns = _project_db_browser_columns(selected_columns, include_raw=include_raw and str(role).lower() in {"admin", "internal"})

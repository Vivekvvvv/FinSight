            ...
            if as_of:
                try:
                    ...
                    if as_of_dt.tzinfo is None:
                        # naive 值是本机墙钟（datetime.now().isoformat() 写入惯例，
                        # 与 task_router/reports_to_review/timeline_service 一致）：
                        # 先按本机时区还原再归一 UTC。直接 replace(tzinfo=utc) 会把
                        # 本机时间错当 UTC 后移 +8h，临界过期报告 days_old 减一天漏报。

...
# Hard floor when persona weights cause an agent score to collapse — keep at
# least one slot reachable so we never *fully* mute a dimension. Combined with
# the registry-side clamp (>= 0.2) this gives a defensive floor.
_PERSONA_WEIGHT_FLOOR = 0.2
...
def _apply_persona_weights(
    ...
    """Re-rank `selection.scores` using persona agent_weights.

    Returns a new selection dict with:
      - `scores` replaced by weighted scores
      - `persona_weights_applied`: dict of {agent_name: weight}
      - `selected` reordered by new scores when `max_agents` is finite

    No-op when persona_config is missing / weights are all 1.0.
    """
    if not isinstance(persona_config, dict):
        return selection
    weights_raw = persona_config.get("agent_weights")
    ...
    # Build weight lookup tolerating both "fundamental" and "fundamental_agent" keys
    def _w(agent_name: str) -> float:
    ...
    for name, base in scores.items():
        try:
            ...
            base_f = 0.0
        w = _w(name)
        weighted[name] = base_f * w
        if abs(w - 1.0) > 1e-6:
            applied[name] = w
            skewed = True

    new_selection = dict(selection)
    new_selection["scores"] = weighted
    if skewed:
        new_selection["persona_weights_applied"] = applied
        # Reorder existing selected list by new score (preserves required agents)
        selected = list(selection.get("selected") or [])
        if selected:
            required = set(selection.get("required") or [])
            # Required agents stay first (in original order); optional re-sorted by score
            required_in_order = [a for a in selected if a in required]
            optional_in_order = [a for a in selected if a not in required]
            optional_in_order.sort(
                key=lambda a: weighted.get(a, 0.0),
                reverse=True,
            )
            new_selection["selected"] = required_in_order + optional_in_order
...
def policy_gate(state: GraphState) -> dict:
    ...
    if agents_override and isinstance(agents_override, list):
        ...
        if force_all_agents:
            ...
            # --- Persona weight re-ranking (no-op when neutral or weights=1.0) ---
            persona_config = state.get("persona_config")
            selection = _apply_persona_weights(selection, persona_config)
            ...
            if "persona_weights_applied" in selection:
                agent_selection["persona_weights_applied"] = selection["persona_weights_applied"]
    ...
    trace.update(
        {
            "policy": {
                ...
                "persona_weights_applied": agent_selection.get("persona_weights_applied"),
                "agent_selection": {

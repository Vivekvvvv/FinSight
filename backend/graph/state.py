...
class GraphState(MessagesState):
    ...
    # --- Persona Mode: investment-style lens ---
    # persona_id: identifier resolved from ui_context.persona_id (e.g. "value_investor").
    # persona_config: serialized Persona (Pydantic .model_dump()) cached on first read,
    #                 consumed by policy_gate (agent_weights), synthesize (lens prompt),
    #                 and report_builder (section priority). Absent / None falls back to
    #                 neutral mode = current behavior.
    persona_id: NotRequired[str]
    persona_config: NotRequired[dict[str, Any]]

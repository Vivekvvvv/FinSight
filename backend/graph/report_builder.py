...
def _build_report_payload_impl(*, state: dict[str, Any], query: str, thread_id: str) -> dict[str, Any] | None:
    ...
    base_report_dict: dict[str, Any] = {
        ...
        "grounding_rate": grounding_rate,
        "generated_at": _now_iso(),
        "persona_id": str(state.get("persona_id") or "neutral"),
        "sections": sections,
        "citations": citations,
        "risks": risks,
        "recommendation": "继续观察并复查证据",
    ...
    if isinstance(validated, dict):
        ...
        # Persona Mode: ReportValidator strips unknown fields, re-inject for UI.
        validated["persona_id"] = str(state.get("persona_id") or "neutral")

...
class PlanBuilder:
    ...
    def _get_timeouts(cls) -> Dict[str, Optional[int]]:
        ...
        for key in list(timeouts.keys()):
            raw_value = os.getenv(f"FINSIGHT_AGENT_TIMEOUT_{key.upper()}")
            ...
            try:
                ...
                continue
            timeouts[key] = timeout if timeout > 0 else None
...
class PlanExecutor:
    ...
    async def execute(
        ...
        def _record_error(step: Optional[PlanStep], message: str) -> None:
            errors.append(message)
            error_details.append({
                "step_id": step.step_id if step else None,
                "agent": step.agent_name if step else None,
        ...
        while pending or running:
            ...
            for step_id, step in list(pending.items()):
                if any(dep in failed for dep in step.depends_on):
                    ...
                    if step.agent_name:
                        _record_error(step, f"{step.agent_name}:dependency_failed")
                    failed.add(step_id)

...
class DeepSearchAgent(BaseFinancialAgent):
    ...
    def _cache_key(ticker: str, query: str) -> str:
        query_digest = hashlib.sha256(str(query or "").encode("utf-8")).hexdigest()[:16]
        return f"{ticker}:deep_search:{query_digest}"
    ...
    async def _initial_search(
        ...
        cache_key = self._cache_key(ticker, query)
...
- 禁止宽泛查询（如"公司近况"），必须针对性
    ...
    def _search_web(self, query: str) -> List[Dict[str, Any]]:
        ...
        if trusted_count < 2:
            try:
                feed_search = getattr(self.tools, "search_authoritative_feeds", None) if self.tools else None
                if not callable(feed_search):
                    return results
